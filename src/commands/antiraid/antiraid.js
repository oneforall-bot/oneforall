    ...
    */
    run: async (oneforall, message, guildData, memberData, args) => {
        ...
        if (subCommand === 'on' || subCommand === 'off') {
            for (const feature of Object.keys(guildData.antiraid.enable)) guildData.antiraid.enable[feature] = subCommand === 'on'
            return guildData.save().then(() => {
                oneforall.functions.tempMessage(message, lang.antiraid.enable.all(subCommand))
            })
        ...
        if (subCommand === "config") {
            ...
            const components = () => {
                return [
                    ...
                    new MessageButton()
                        ...
                        .setEmoji('▶️')
                        .setStyle('SECONDARY'),
                    new MessageSelectMenu().setOptions(Object.keys(guildData.antiraid.config).slice(slicerIndicatorMin, slicerIndicatorMax).map((feature) => {
                        return {
                            value: feature,
                            label: feature
                        }
            ...
            const componentFilter = {
                filter: interaction => interaction.customId.includes(`antiraid`) && interaction.customId.includes(message.id) && interaction.user.id === message.author.id,
                time: 900000
            },
                awaitMessageFilter = {
            ...
            const { config, enable, limit } = tempConfig
                ...
                if (interaction.componentType === 'BUTTON') {
                    ...
                    if(selectFeature) {
                        ...
                        if (selectOption === 'limit') {
                            const selectMenu = lang.antiraid.config.configMenu(enable[selectFeature], isLimit).find(opts => opts.value === selectOption)
                            const questionAnswer = await generateQuestion(selectMenu.question)
                            if (selectFeature === 'antiMassBan' || selectFeature === 'antiMassKick' || selectFeature === 'antiLink' || selectFeature === 'antiToken' || selectFeature === 'antiMassMention') {
                                if (!oneforall.functions.isValidTime(questionAnswer.content, true)) {
                                    return oneforall.functions.tempMessage(message, lang.antiraid.limit.errorNotNumber)

                                }
                            }
                            if (selectFeature === 'antiDc') {
                                if (!oneforall.functions.isValidTime(questionAnswer.content)) {
                                    return oneforall.functions.tempMessage(message, lang.antiraid.limit.errorAntiDc)
                                }
                            }
                        ...
                        if(selectOption === 'sanction'){
                            const selectMenu = lang.antiraid.config.configMenu(enable[selectFeature], isLimit).find(opts => opts.value === selectOption)
                            const questionAnswer = await generateQuestion(selectMenu.question)
                            if(questionAnswer.content !== 'mute' && questionAnswer.content !== 'unrank' && questionAnswer.content !== 'kick' && questionAnswer.content !== 'ban') return oneforall.functions.tempMessage(message, lang.antiraid.config.wrongSanctionType)
                            config[selectFeature] = questionAnswer.content
                        }
                    }
                    return await panel.edit({
                        embeds: [embedPageChanger(page)],
                        components: component.map(c => new MessageActionRow({ components: [c] }))
                    })
                }
                await panel.edit({
                    embeds: [embedPageChanger(page)],
