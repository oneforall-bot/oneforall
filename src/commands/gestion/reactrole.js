const {MessageActionRow, MessageSelectMenu, MessageEmbed, Collection, Util} = require('discord.js')

module.exports = {
    name: 'reactrole',
    description: 'React role menu',
    usage: 'reactrole',
    cooldown: 3000,
    run: async (oneforall, message, guildData, memberData, args) => {
        const hasPermission = memberData.permissionManager.has("REACTROLE_CMD");
        if(!hasPermission) return message.reply(guildData.langManager.notEnoughPermissions('reactrole'))
        ...
        const reactRole = {channel: guildData.langManager.undefined, message: guildData.langManager.undefined, emojiRoleMapping: new oneforall.Collection()}
        const embed = {
            title: 'Configuration',
            fields: [
                {
        ...
        const componentFilter = {
                filter: interaction => interaction.customId === `reactrole.${message.id}` && message.author.id === interaction.user.id,
                time: 900000
            },
            awaitMessageFilter = {
        ...
        collector.on('collect', async (interaction) => {
            ...
            if (selectedOption === 'add-role') {
                ...
                if (oneforall.functions.roleHasSensiblePermissions(role.permissions)) return errorMessage(guildData.langManager.reactrole.tryToPermsRole)
