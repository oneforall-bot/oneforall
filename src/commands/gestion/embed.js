...
module.exports = {
    name: 'embed',
    description: 'Create embed easy',
    usage: 'embed [number of embed to create]',
    cooldown: 3 * 1000,
    run: async (oneforall, message, guildData, memberData, args) => {
        ...
        if(!hasPermission) return message.reply(lang.notEnoughPermissions('embed'))
        const numberOfEmbed = args[0]
        let embeds = []
        let defaultOptions = lang.embedBuilder.baseMenu
        if (numberOfEmbed && numberOfEmbed > 1) {
            if (parseInt(numberOfEmbed) > 10) return oneforall.functions.tempMessage(message, lang.embedBuilder.invalidNumberOfEmbed)
            defaultOptions = []
            for (let i = 0; i < parseInt(numberOfEmbed); i++) {
                embeds.push({
        ...
        const componentFilter = {
                filter: interaction => interaction.customId === `embed.${message.id}` && message.author.id === interaction.user.id,
                time: 900000
            },
            awaitMessageFilter = {
        ...
        const collector = panel.channel.createMessageComponentCollector(componentFilter);
        let selectedEmbed = 0
        collector.on('collect', async (interaction) => {
            ...
            if (selectedOption.includes('back')) {
                if (selectedOption === 'back')
                    ...
                    return updateOptions(interaction, 'Create your embed', !numberOfEmbed || parseInt(numberOfEmbed) === 1 ? lang.embedBuilder.baseMenu : [...lang.embedBuilder.baseMenu, {
                        label: 'Back',
                        value: 'back',
                        description: 'Go to back to the embed selector',
                        emoji: '↩'
            ...
            if (menuSelected.questionOnly) {
                ...
                if (menuSelected.value === 'color') {
                    ...
                    if (!color) return oneforall.functions.tempMessage(message, lang.embedBuilder.errorColor)
                ...
                if (menuSelected.value !== 'send') {
                    ...
                    collector.stop()
                    panel.delete()
                    return oneforall.functions.tempMessage(message, `Embed(s) sent in ${channel.toString()}`)
                }
            } else {
