...
module.exports = {
    ...
    run: async (oneforall, interaction, memberData, guildData) => {
        ...
        const row = new MessageActionRow()
            .addComponents(
                new MessageSelectMenu()
                    .setCustomId(`reactrole.${interaction.id}`)
        ...
        const componentFilter = {
                filter: interactionReactrole => interactionReactrole.customId === `reactrole.${interaction.id}` && interactionReactrole.user.id === interaction.user.id,
                time: 900000
            },
            awaitMessageFilter = {
        ...
        collector.on('collect', async (interactionReactrole) => {
            ...
            if (selectedOption === 'message') {
                ...
                if (reactRole.channel === guildData.langManager.undefined || !interaction.guild.channels.cache.get(reactRole.channel)) return errorMessage(guildData.langManager.reactrole.noChannel)
                let fetchedMessage;
                try {
                    fetchedMessage = await interaction.guild.channels.cache.get(reactRole.channel).messages.fetch(content)
                } catch (e) {
            ...
            if (selectedOption === 'add-role') {
                ...
                const role = questionAnswerRole.mentions.roles.first() || interaction.guild.roles.cache.get(questionAnswerRole.content)
                if (!role || role.id === interaction.guild.roles.everyone.id) return errorMessage(guildData.langManager.reactrole.roleNotValid)
                ...
                if (emoji.id) {
                    ...
                    if (!tempEmoji) {
                        ...
                        emoji = await interaction.guild.emojis.create(link, name, {reason: `emoji reaction role by ${interaction.user.username + '#' + interaction.user.discriminator}`})
                    } else {
                        emoji = tempEmoji
                    }
                }
            ...
            if (selectedOption === 'delete') {
                ...
                const fetchMessage = await interaction.guild.channels.cache.get(toDelete.channel).messages.fetch(toDelete.message);
                if (fetchMessage) {
            ...
            if (selectedOption === 'save') {
                ...
                const fetchedMsg = await interaction.guild.channels.cache.get(reactRole.channel).messages.fetch(reactRole.message)
                for (const [key, value] of reactRole.emojiRoleMapping) {
