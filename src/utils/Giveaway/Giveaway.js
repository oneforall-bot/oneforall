...
class Giveaway extends EventEmitter {
    ...
    async checkWinnerEntry(user) {
        ...
        const member = await this.message.guild.members.fetch(user.id, {withPresences: true}).catch(() => {});
     ...
     */
        ...
        return new Promise(async (resolve, reject) => {
            ...
            if (winners.length > 0) {
                ...
                const message = winMessage?.replace('{winners}', formattedWinners).replace('{prize}', this.options.prize);
                ...
                if (this.messages.winMessage.embed && typeof this.messages.winMessage.embed === 'object') {
                    ...
                    const embedDescription = embed.description.replace('{winners}', formattedWinners).replace('{user}', this.hostedBy)
