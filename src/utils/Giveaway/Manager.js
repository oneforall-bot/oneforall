...
class GiveawaysManager extends EventEmitter {
            ...
            .setDescription(
                giveaway.isDrop
                    ...
                    : (
                        ...
                        giveaway.messages.inviteToParticipate.replace('{reaction}', Discord.Util.resolvePartialEmoji(giveaway.reaction) ? giveaway.reaction : undefined) +
                        '\n' +
    ...
    generateNoValidParticipantsEndEmbed(giveaway) {
        const embed = new Discord.MessageEmbed()
            ...
            .setDescription(giveaway.messages.noWinner.replace('{user}', giveaway.hostedBy))
            .setTimestamp(new Date(giveaway.endAt).toISOString())
            .setThumbnail(giveaway.thumbnail);
        return giveaway.fillInEmbed(embed);
    }
    ...
    async deleteGiveaway(messageId) {
        return new Promise((resolve, reject) => {
            this.client.managers.giveawaysManager.getAndCreateIfNotExists(messageId, {messageId}).delete()
            this.refreshStorage()
            resolve(true)
        });
    }
    ...
    async editGiveaway(messageID, giveawayData) {
        return new Promise((resolve, reject) => {
            const giveaway = this.client.managers.giveawaysManager.find(giveaway => giveaway.messageId === messageID)
    ...
    async saveGiveaway(messageId, giveawayData) {
        return new Promise((resolve, reject) => {
            this.client.managers.giveawaysManager.getAndCreateIfNotExists(messageId, {
                messageId,
                data: giveawayData
            }).save().then(() => {
                resolve(true)
    ...
    async _handleRawPacket(packet) {
        ...
        const member = await guild.members.fetch(packet.d.user_id, {withPresences: true}).catch(() => {
        });
        if (!member) return;
        ...
        if (packet.t === 'MESSAGE_REACTION_ADD') {
            if (giveaway.ended) return this.client.emit('endedGiveawayReactionAdded', giveaway, member, reaction);
        ...
        } else this.client.emit('giveawayReactionRemoved', giveaway, member, reaction);
    }
