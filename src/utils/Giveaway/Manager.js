...
class GiveawaysManager extends EventEmitter {
  ...
  constructor(client, options, init = true) {
    ...
    if (
      !new Discord.Intents(client.options.intents).has(
        Discord.Intents.FLAGS.GUILD_MESSAGE_REACTIONS
      )
    ) {
      throw new Error(
        'Client is missing the "GUILD_MESSAGE_REACTIONS" intent.'
      );
    }
  ...
  generateMainEmbed(giveaway, lastChanceEnabled = false) {
      ...
      .setDescription(
        giveaway.isDrop
          ...
          : (giveaway.pauseOptions.isPaused
              ...
              : "") +
              giveaway.messages.inviteToParticipate.replace(
                "{reaction}",
                Discord.Util.resolvePartialEmoji(giveaway.reaction)
                  ? giveaway.reaction
                  : undefined
              ) +
              "\n" +
    ...
    if (giveaway.endAt !== Infinity)
      embed.setTimestamp(new Date(giveaway.endAt).toISOString());
  ...
  generateNoValidParticipantsEndEmbed(giveaway) {
    const embed = new Discord.MessageEmbed()
      ...
      .setDescription(
        giveaway.messages.noWinner.replace("{user}", giveaway.hostedBy)
      )
      .setTimestamp(new Date(giveaway.endAt).toISOString())
      .setThumbnail(giveaway.thumbnail);
    return giveaway.fillInEmbed(embed);
    ...
    return new Promise(async (resolve, reject) => {
      ...
      if (
        ...
        !channel
          ...
          ?.has([
            channel.locked ||
            (!channel.joined && channel.type === "GUILD_PRIVATE_THREAD")
      ...
      const giveaway = new Giveaway(this, {
        ...
        reaction: Discord.Util.resolvePartialEmoji(options.reaction)
          ? options.reaction
          : undefined,
        botsCanWin:
        ...
        lastChance:
          options.lastChance && typeof options.lastChance === "object"
            ? options.lastChance
            : undefined,
        pauseOptions:
          options.pauseOptions && typeof options.pauseOptions === "object"
          ...
          .awaitReactions({
            filter: (r, u) =>
              [r.emoji.name, r.emoji.id]
                .filter(Boolean)
                .includes(reaction.emoji.name || reaction.emoji.id) &&
  ...
  async deleteGiveaway(messageId) {
    return new Promise((resolve, reject) => {
      this.client.managers.giveawaysManager
        .getAndCreateIfNotExists(messageId, { messageId })
        .delete();
      this.refreshStorage();
      resolve(true);
    });
  ...
  async getAllGiveaways() {
    return this.client.managers.giveawaysManager.map((giveaway) => {
  ...
  async editGiveaway(messageID, giveawayData) {
    return new Promise((resolve, reject) => {
      const giveaway = this.client.managers.giveawaysManager.find(
        (giveaway) => giveaway.messageId === messageID
      );
      if (!giveaway) reject("Giveaway not found");
      giveaway.data = giveawayData;
      giveaway.save();
      resolve(true);
      this.refreshStorage();
    });
  ...
  async saveGiveaway(messageId, giveawayData) {
    return new Promise((resolve, reject) => {
      this.client.managers.giveawaysManager
        .getAndCreateIfNotExists(messageId, {
          messageId,
          data: giveawayData,
        })
        .save()
    ...
    this.giveaways.forEach(async (giveaway) => {
      ...
      if (giveaway.isDrop) {
        ...
        reaction = giveaway.message?.reactions.cache.find((r) =>
          [r.emoji.name, r.emoji.id]
            .filter(Boolean)
            .includes(reaction?.name || reaction?.id)
        );
  ...
  async _handleRawPacket(packet) {
    ...
    const member = await guild.members
      .fetch(packet.d.user_id, { withPresences: true })
      .catch(() => {});
    if (!member) return;
    ...
    if (packet.t === "MESSAGE_REACTION_ADD") {
      ...
      this.client.emit("giveawayReactionAdded", giveaway, member, reaction);
      if (giveaway.isDrop && reaction.count - 1 >= giveaway.winnerCount)
        this.end(giveaway.messageId).catch(() => {});
    } else
      this.client.emit("giveawayReactionRemoved", giveaway, member, reaction);
  }
