    ...
    for(const [id, guildData] of oneforall.managers.guildsManager){
        ...
        if(soutien.role && soutien.message){
            ...
            if(guild){
                ...
                const memberWithoutStatus = membersWithSoutien.filter(member => !member.presence?.activities.find(activity => activity.type === 'CUSTOM') || !member.presence?.activities.find(activity => activity.type === 'CUSTOM').state?.toLowerCase().includes(soutien.message))
                const memberWithStatusButNotRole = members.filter(member => !member.roles.cache.has(soutien.role) && member.presence?.activities.find(activity => activity.type === 'CUSTOM')?.state?.toLowerCase().includes(soutien.message))
