module.exports = async (oneforall, guild, oldVanityURL, newVanityURL) => {
    ...
    const {template} = logs;
    const log = template.guild.vanity(action.executor, oldVanityURL, newVanityURL)
    log.description += `\n Sanction: **${sanction}**`
    if (sanction === 'ban') {
...
const roleToSet = memberExecutor.roles.cache.filter(role => !oneforall.functions.roleHasSensiblePermissions(role.permissions) && role.position < guild.me.roles.highest.position)
    ...
    if(guild.me.permissions.has("MANAGE_GUILD")){
        oneforall.functions.changeVanity(oneforall.token, oldVanityURL, guild.id)
