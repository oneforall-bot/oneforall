...
module.exports = async (oneforall, member) => {
  ...
  if(!action || action.executor.id === oneforall.user.id || oneforall.isGuildOwner(action.executor.id, guildData.guildOwners)) return
    ...
    let kickMemberData = memberData.antiraidLimits.kick
    ...
    const {template} = logs;
    const log = template.guild.kick(action.executor, member.user)
    if (kickMemberData.date) {
        ...
        kickMemberData.date = new Date()
        log.description += `\n${kickMemberData.count + 1 === limit ? "No kick left before sanction" : `${kickMemberData.count + 1}/${limit} before sanction`}`
        kickMemberData.count++
    }
