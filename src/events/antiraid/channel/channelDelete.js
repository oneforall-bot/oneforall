module.exports = async (oneforall, channel) => {
    ...
    const action = await guild.fetchAuditLogs({type: "CHANNEL_DELETE"}).then(async (audit) => audit.entries.first());
    ...
    const {template} = logs;
    const log = template.channel.delete(action.executor, channel)
    log.description += `\n Sanction: **${sanction}**`
    if (sanction === 'ban') {
