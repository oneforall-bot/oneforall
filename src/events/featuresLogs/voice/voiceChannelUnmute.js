module.exports = async (oneforall, member, muteType) => {
    ...
    if(diff && diff < 1000 ){
        ...
        return channel.send({embeds : [template.voice.unmute(executor, member, member.voice.channel, muteType)]})

    }
    channel.send({embeds : [template.voice.unmute(member, member, member.voice.channel, muteType)]})
}
